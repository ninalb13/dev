	...
	/* frontier = Queue()
		...
		visited = {}
			...
			while not frontier.empty():
				...
				visited[next] = True
			*/
	//int aux_front = frontier.Count();
	if (frontier.Count() != 0) 
	{
		iPoint curr;
	//save the frontier to visited
			p2Queue_item<iPoint>* aux = frontier.start;
			visited.add(aux->data);
			frontier.Pop(aux->data);
	//calculate 4 neigh
				//right
			curr = aux;
				curr.create(aux->data.x + 1, aux->data.y);
				...
				//top
				curr.create(aux->data.x , aux->data.y+1);
				if (visited.find(curr) == -1) {
					frontier.Push(curr);
					visited.add(curr);
				}
...
bool j1Map::IsWalkable(int x, int y) const
{
	...
	//if (x >= 0 && x <= (int)data.width && y >= 0 && y <= (int)data.height)
	//{
	//	return true;
